    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="relative w-full max-w-md bg-[#121a28] border-2 border-[#d4af37] rounded-3xl shadow-2xl p-6 sm:p-8 text-slate-100 font-serif-vintage overflow-hidden">
        ...
        {!currentProfile.isGuest ? (
          <div className="bg-[#172437] border border-[#d4af37]/40 rounded-2xl p-4 mb-2 shadow-inner">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                {currentProfile.avatar.startsWith('http') ? (
                  <img src={currentProfile.avatar} alt="avatar" className="w-11 h-11 rounded-full object-cover border-2 border-[#d4af37]" />
                ) : (
                  <div className="w-11 h-11 rounded-full bg-[#0e1624] border border-[#d4af37]/40 flex items-center justify-center text-xl">
                    {currentProfile.avatar}
                  </div>
                )}
                <div className="text-left">
                  <div className="font-bold text-sm text-slate-100">{currentProfile.name}</div>
                  <div className="text-xs text-slate-400 font-mono">{currentProfile.email}</div>
                  <span className="inline-flex items-center gap-1 text-[10px] text-emerald-400 font-mono mt-0.5">
                    <UserCheck className="w-3 h-3" /> Авторизовано через Google
                  </span>
                </div>
              </div>
            </div>
            <div className="flex items-center justify-between pt-3 border-t border-slate-700/60 text-xs font-mono">
              <div className="text-slate-300">
                Дедуктивний досвід: <b className="text-[#f5d77f]">{currentProfile.xp} XP</b>
              </div>
              <button
                onClick={handleLogout}
                className="px-3 py-1.5 text-xs bg-red-950/50 border border-red-500/40 text-red-300 rounded-xl hover:bg-red-900/60 transition-colors flex items-center gap-1.5 font-mono"
              >
          ...
          <>
            ...
            >
              ...
              <span>{loading ? 'З\'єднання з Google...' : 'Продовжити через Google'}</span>
            </button>
